...
const TechIcon = ({ tech, index }) => {
    return (
        <motion.div
            className="relative group flex flex-col items-center gap-2 px-4"
            animate={{
                y: [0, -40, 0, 40, 0], // Even bigger curves
            }}
            transition={{
        ...
        >
            ...
            <span className="text-[10px] md:text-[12px] font-secondary font-bold text-foreground/40 uppercase tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                {tech.name}
            </span>
        </motion.div>
    );
